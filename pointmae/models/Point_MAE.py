...
import numpy as np
from pointmae.models.build import MODELS
from pointmae.utils import misc
from pointmae.utils.checkpoint import get_missing_parameters_message, get_unexpected_parameters_message
from pointmae.utils.logger import *
import random
from knn_cuda import KNN
from pointmae.extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL2
...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, points):
        ...
        # fps the centers out
        center = misc.fps(points, self.num_group)[:, :, :3]  # B G 3
        # knn to get the neighborhood
        _, idx = self.knn(points[:, :, :3], center)  # B G M
        assert idx.size(1) == self.num_group
        assert idx.size(2) == self.group_size
        idx_base = torch.arange(0, batch_size, device=points.device).view(-1, 1, 1) * num_points
        idx = idx + idx_base
        idx = idx.view(-1)
        neighborhood = points.view(batch_size * num_points, -1)[idx, :]
